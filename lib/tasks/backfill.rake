namespace :backfill do
  ...
  task fees_charges: [:environment] do
    Merchant.find_each { Merchants::CalculateFeesChargesService.call(_1) }
  end
end
...
task year_to_year_report: [:environment] do
  ...
  orders_fees_by_year = Disbursement.group("DATE_PART('Year', date)").sum(:total_fees)
  ...
  report = years.map do |year|
    {
      ...
      "Amount of order fees" => "#{orders_fees_by_year[year].to_f} €",
